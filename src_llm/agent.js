...
export class IntentionRevisionRevise extends IntentionRevision {
    ...
    getUtility ( predicate ) {
        ...
        if ( action === 'handoff_llm' )         return 9999;
    ...
    async push ( predicate ) {
        ...
        // At most one edge objective per edge; replace if closest tile changed.
        else if ( (action === 'go_to_bonus' || action === 'drop_on_tile') && predicate[3] ) {
            const edgeId = predicate[3];
    ...
    pushUrgent ( predicate ) {
        const currentIsHandoff = this.intention_queue.length > 0 &&
            this.intention_queue[ 0 ].predicate[ 0 ] === 'handoff_llm';
